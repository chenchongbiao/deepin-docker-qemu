...
finish() {
  ...
  if [ -f "$QEMU_PID" ]; then
    ...
    pid=$(<"$QEMU_PID")
    error "Forcefully terminating deepin, reason: $reason..."
...
_graceful_shutdown() {
  ...
  if ! ready; then
    info "Cannot send ACPI signal during deepin setup, aborting..."
    finish "$code" && return "$code"
  fi
  ...
  while [ "$cnt" -lt "$QEMU_TIMEOUT" ]; do
    ...
    info "Waiting for deepin to shutdown... ($cnt/$QEMU_TIMEOUT)"
